                  ...
                  <div class="card-body card-dashboard">
                    ...
                    <table class="table table-striped table-bordered zero-configuration">
                      <thead> 
                        ...
                        <tbody>
                          <!--{{$s_no=1}}-->
                          @foreach($processing_orders as $process)
                          <tr>
                            <td>{{$s_no}}</td>
                            <td>{{$process->order_id}}</td>
                            <td>{{$process->user_name}}</td>
                            <td>{{$process->res_name}}</td>
                            ...
                            switch ((int) $process->order_status) {
                              case 0:
                                echo 'Order Placed';
                              break;
                              case 1:
                                echo 'Restaurant Accepted';
                              break;
                              case 2:
                            ...
                            ?></button></td>
                            <td>{{$process->user_phone}}</td>
                            <td>{{$process->res_phone}}</td>
                            <td>{{$process->driver_phone}}</td>
                           
                            <td>
                          
                           <button class="btn btn-primary btn-sm"><i class="ft-edit"></i><a style=" color: white !important;" href="{{URL('/')}}/admin/view_areas/{{$process->id}}"> View</a></button>
                             </td>
                          </tr>
                          <!--{{$s_no++}}-->
                          @endforeach
